  ...
  module Taxable
    module ActMethods
      def acts_as_taxable(*args)
        ...
        taxable_class_info.new_args.each do |method_name|
        ...
        # Register callbacks if this is the first time acts_as_taxable has been called
        if taxable_class_info.previous_info.nil?
        ...
        define_method(method_name + "_taxed") do |*args|
          ...
          if @taxed_attr_values[method_name]
            ...
            if value.nil?
              ...
              #::Invoicing::Utils.round_to_currency_precision(taxed) # FIXME
              taxed
            end
          end
        end
