module Invoicing
  ...
  module LineItem
    module ActMethods
      ...
      def acts_as_line_item(*args)
        ...
        if info.previous_info.nil? # Called for the first time?
          ...
          acts_as_currency_value(info.method(:net_amount), info.method(:tax_amount), :gross_amount)
    ...
    # The sum of +net_amount+ and +tax_amount+. The formatted value can be obtained through
    # +gross_amount_formatted+ (method generated by +CurrencyValue+).
    def gross_amount
      line_item_class_info.get(self, :net_amount) + line_item_class_info.get(self, :tax_amount)
    end
    alias_method :gross_amount_before_type_cast, :gross_amount
