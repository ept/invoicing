module Invoicing
  ...
  module LedgerItem
    ...
    module ActMethods
      ...
      def acts_as_ledger_item(*args)
        ...
        if info.previous_info.nil? # Called for the first time?
          ...
          acts_as_currency_value(info.method(:total_amount), info.method(:tax_amount), :net_amount,
    ...
    # The difference +total_amount+ minus +tax_amount+. The formatted value can be obtained through
    # +net_amount_formatted+ (method generated by +CurrencyValue+).
    def net_amount
      ledger_item_class_info.get(self, :total_amount) - ledger_item_class_info.get(self, :tax_amount)
    end
    alias_method :net_amount_before_type_cast, :net_amount
