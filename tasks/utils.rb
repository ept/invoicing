...
module Utils
  class << self
    ...
    def announcement
      ...
      { 
        :subject        => "#{cfg.name} #{Invoicing::VERSION} Released",
        :title          => "#{cfg.name} version #{Invoicing::VERSION} has been released.",
        :urls           => "#{cfg.homepage}",
        :description    => "#{cfg.description.rstrip}",
        :release_notes  => Utils.release_notes_from(cfg.history)[Invoicing::VERSION].rstrip 
      }
    end
  end
end # << self
